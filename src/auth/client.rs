...
impl ClientAuth {
    ...
    pub fn update(&self, key: &str, value: &str)->bool{
        ...
        match key {
            "uid"=> return false,
            "apikey"=> updating_client.apikey = value.to_string(),
            "salt"=> updating_client.salt = value.to_string(),
            "username"=> updating_client.username = value.to_string(),
            "pass512"=> updating_client.pass512 = value.to_string(),
            "email"=> updating_client.email = value.to_string(),
            "email_token"=> updating_client.email_token = value.to_string(),
            "email_expiry"=> updating_client.email_expiry = value.to_string(),
            "public_key"=> updating_client.public_key = value.to_string(),
            "totp_key"=> updating_client.totp_key = value.to_string(),
            "level"=> updating_client.level = AuthLevel::from_str(value).unwrap(),
            _=>return false
        };
        let root = database::get_root(database::CLIENT).unwrap();
