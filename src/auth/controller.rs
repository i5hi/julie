...
pub fn update_basic_auth(mut storage: impl JulieStorage, uid: String, username: &str, password: &str)->bool{
...
/// 
/// 
pub fn get_client(mut client_storage: impl JulieStorage, uid: String)->Result<ClientAuth,S5ErrorKind>{
...
pub fn get_service(mut service_storage: impl JulieStorage, name: String)->Result<ServiceIdentity,S5ErrorKind>{
...
pub fn check_apikey(mut client_storage: impl JulieStorage,uid: String, apikey: String) -> Result<bool,S5ErrorKind> {
...
mod tests {
    ...
    fn core_composite() { 
        ...
        let registered_client = update_basic_auth(client_storage.clone(),client_auth.clone().uid, username, &p256.clone());
        ...
        let signatory_client = update_public_key(client_storage.clone(),client_auth.clone().uid, public_key);
        let message = "timestamp=now";
        let updated = get_client(client_storage.clone(),client_auth.uid).unwrap();
        ...
        let mfa_client = update_totp_key(client_storage.clone(),updated.clone()).unwrap();
    ...
    fn core_email_composite(){
        ...
        let _client_auth = update_email(storage.clone(),client_auth.clone().uid, email);
