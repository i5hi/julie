...
pub async fn handle_put_basic(
    ...
    client_storage: Box<dyn JulieStorage>,
) -> Result<impl warp::Reply, warp::Rejection> {
    if uid == "".to_string() {
        return Err(warp::reject::custom(S5ErrorKind::UID))
    }
    else if apikey == "".to_string() {
        return Err(warp::reject::custom(S5ErrorKind::ApiKey))
    }
    match controller::check_apikey(client_storage.clone_box(),apikey,uid.clone()){
        Ok(_)=>{
...
// ) -> Result<impl warp::Reply, warp::Rejection> {
//     match controller::check_apikey(client_storage,apikey,uid.clone()){
//         Ok(_)=>{
//             if controller::update_email(dup_storage.clone(), uid, &auth_email.email){
