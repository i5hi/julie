...
pub async fn handle_put_basic(client:Result<ClientAuth,warp::Rejection>, auth_basic: AuthBasic) -> Result<impl warp::Reply,warp::Rejection>{
    let client =  client?;
...
pub async fn handle_put_pubkey(client:Result<ClientAuth,warp::Rejection>, auth_pubkey: AuthPublicKey) -> Result<impl warp::Reply,warp::Rejection>{
    let client =  client?;
    let client = core::update_public_key(client,&auth_pubkey.public_key);
    Ok(warp::reply::json(&client))
}
/// Handle a warp http request to get a totp_key.
pub async fn handle_get_totp_key(client:Result<ClientAuth,warp::Rejection>) -> Result<impl warp::Reply,warp::Rejection>{
    let client =  client?;
    let client = core::update_totp_key(client).unwrap();
    Ok(warp::reply::json(&client))
}
/// Handle a warp http request to post an otp to establish a totp_key.
pub async fn handle_post_totp(client:Result<ClientAuth,warp::Rejection>, otp: u64) -> Result<impl warp::Reply,warp::Rejection>{
    let client =  client?;
...
pub async fn handle_get_token(client:Result<ClientAuth,warp::Rejection>, service: String) -> Result<impl warp::Reply,warp::Rejection>{
    let client =  client?;
    let token = AuthToken{
        token: core::issue_token(client,&service).unwrap()
    };
...
pub fn filter_totp(client: Result<ClientAuth,warp::Rejection>, otp: u64)-> Result<ClientAuth,warp::Rejection>{
    
    let client =  client?;
    let verify = core::verify_totp(client.clone(),otp);
