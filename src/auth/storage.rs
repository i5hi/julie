...
pub enum AuthLevel{
    /// ApiKey is level 1, when only a uid and apikey is generated by an admin and shared to a client to register their account.
    ApiKey,
    /// Basic signifies a username:pass256 set by the client (requires ApiKey).
    Basic,
    /// Signature signifies a public_key is registered; requiring requests to be signed by the client (requires Basic). 
    Signature,
    /// Totp is if a shared_secret is verified by the client (requires Basic).
    Totp,
...
impl ClientAuth {
    ...
    pub fn new() -> Self {
        let root = database::get_root(database::AUTH_TREE).unwrap();
        let uid = format!("s5id-{}", Uuid::new_v4());
